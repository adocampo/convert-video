...
function Install-ScheduledTask {
    ...
    if (-not $isAdmin) {
        ...
        $elevatedLines = @(
            ...
            '    Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -User $user -Password $plainPass -Description "Run clutch media transcoding service at system startup" -ErrorAction Stop | Out-Null'
        ...
        $elevatedScript = $elevatedLines -join $nl
