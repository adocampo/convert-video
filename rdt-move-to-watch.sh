#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────
# rdt-move-to-watch.sh — rdt-client post-download script
#
# Moves downloaded video files into watch directories so clutch can
# pick them up for conversion.  Unlike the qBittorrent variant there
# is no need to keep seeding, so files are moved instead of hardlinked.
#
# rdt-client "Run on torrent finished" command:
#   /path/to/rdt-move-to-watch.sh "%N" "%L" "%F"
#
#   %N = Torrent name
#   %L = Category
...
LOG_FILE="/config/rdt-move-to-watch.log"   # set to a path to enable logging
# ──────────────────────────────────────────────────────────────────────
...
classify() {
    ...
    # PACK, serie (case-insensitive), or S01..S99 pattern → series
    if [[ "$name_upper" =~ PACK ]] \
    || [[ "$name_upper" =~ SERIE ]] \
    || [[ "$name_upper" =~ S[0-9]{2,} ]]; then
        echo "series"
        return
    fi
